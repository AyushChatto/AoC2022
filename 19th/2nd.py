...
def recurse(node):
    ...
    if node[t] == 0:
        ...
        # Enough ore to make clay robot and not enough clay robots AND NOT ENOUGH FOR OBSIDIAN ROBOTS
        if node[o] >= cb[1] and node[cR] < cmax[1] and (node[c] < cb[3] or node[o] < cb[2]):
            temp2 = temp[:]
            temp2[o] -= cb[1]
            temp2[cR] += 1
            ans.append(recurse(temp2))

        # Enough ore and clay to make obsidian robots, and not enough obsidian robots AND NOT ENOUGH FOR GEODE ROBOTS
        if node[o] >= cb[2] and node[c] >= cb[3] and node[bR] <= cmax[2] and (node[b] < cb[5] or node[o] < cb[4]):
            temp3 = temp[:]
            temp3[o] -= cb[2]
            temp3[c] -= cb[3]
            temp3[bR] += 1
